...
import { fulfilledContactEmail } from "../server/email-templates.js";
...
async function sendFulfillment(env, order, artifacts, requestId) {
  ...
  const previewImageUrl = artifacts.find((artifact) => artifact.previewImageUrl)?.previewImageUrl || null;
  const sections = artifacts.flatMap((artifact) => artifact.sections);
...
async function handleStripeWebhook(request, env, requestId) {
  ...
  try {
    ...
    const items = checkoutOrderItems(products).map((item, index) => ({
      ...item,
      artifactVersion: artifacts[index].artifactVersion,
    }));
    const order = {
